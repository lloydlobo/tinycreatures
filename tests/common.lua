...
describe('common - datastructures', function()
    describe('enumerations', function()
        ...
        it('should have correct health transition values', function()
            assert.are.equal(common.HEALTH_TRANSITIONS.NONE, -1)
            assert.are.equal(common.HEALTH_TRANSITIONS.HEALING, 0)
            assert.are.equal(common.HEALTH_TRANSITIONS.HEALTHY, 1)
        end)
    end)
end)

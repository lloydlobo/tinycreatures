    ...
    do
        ...
        if is_temporary_print_trail_stats then
            for i = 1, MAX_PLAYER_TRAIL_COUNT do
                print('happy trails to yous', player_trails_x[i], player_trails_y[i], player_trails_vel_x[i],
                    player_trails_vel_y[i], player_trails_rot_angle[i], player_trails_is_active[i],
    ...
    for laser_index = 1, #cs.lasers_x do
        if cs.lasers_is_active[laser_index] == common.Status.active then
            ...
            if cs.lasers_time_left[laser_index] <= 0 then -- Deactivate if animation ends
                ...
                cs.lasers_x[laser_index] = cs.lasers_x[laser_index] + math.cos(angle) * config.LASER_PROJECTILE_SPEED *
                    dt
                cs.lasers_y[laser_index] = cs.lasers_y[laser_index] + math.sin(angle) * config.LASER_PROJECTILE_SPEED *
                    dt
                ...
                elseif --[[Deactivate if it goes off screen]] cs.lasers_x[laser_index] < 0 or cs.lasers_x[laser_index] >=
                    arena_w or cs.lasers_y[laser_index] < 0 or cs.lasers_y[laser_index] >= arena_h then
                    cs.lasers_is_active[laser_index] = common.Status.not_active
                end
            end
        end
...
function update_creatures_this_frame(dt)
    ...
    local creature_circle = {
        ...
        y = 0,
        radius = 0
    } ---@type Circle # hope for cache-locality
    for i = 1, TOTAL_CREATURES_CAPACITY do
...
function draw_player(alpha)
    ...
    if curr_state.player_invulnerability_timer > 0 then -- eye winces and widens
        player_iris_radius = lerp(player_iris_radius, (player_iris_radius * 1.328),
            curr_state.player_invulnerability_timer * alpha)
    end
    ...
    do -- @juice ─ simulate the twinkle in eye to go opposite to player's direction
        ...
        if love.keyboard.isDown('up', 'w') then
            inertia_x = curr_state.player_vel_x + math.cos(curr_state.player_rot_angle) * config.PLAYER_ACCELERATION *
                game_timer_dt
            inertia_y = curr_state.player_vel_y + math.sin(curr_state.player_rot_angle) * config.PLAYER_ACCELERATION *
                game_timer_dt
        end
        ...
        player_edge_x = player_edge_x - (dfactor * amplitude_factor * config.PLAYER_FIRING_EDGE_MAX_RADIUS) *
            (inertia_x * game_timer_dt)
        player_edge_y = player_edge_y - (dfactor * amplitude_factor * config.PLAYER_FIRING_EDGE_MAX_RADIUS) *
            (inertia_y * game_timer_dt)
    end
    local invultimer = curr_state.player_invulnerability_timer
    local player_trigger_radius = lerp(config.PLAYER_FIRING_EDGE_RADIUS - 4, config.PLAYER_FIRING_EDGE_RADIUS - 3, --
        alpha + (invultimer * 0.5))
    LG.setColor(common.Color.player_entity_firing_edge_dark)
    LG.circle('fill', player_edge_x, player_edge_y, (invultimer > 0 and
        lerp(player_trigger_radius - (3 * invultimer), player_trigger_radius + (3 * invultimer), invultimer) or
        player_trigger_radius))
end
...
function draw_player_health_bar(alpha)
    ...
    if player_invulnerability_timer > 0 then
        ...
        LG.rectangle('fill', bar_x, bar_y + bar_height * sh, (bar_width * invulnerable_tween) * sw,
            invulnerability_bar_height * sh) -- current invulnerability
    else
        ...
        if curr_state.creatures_is_active[i] == common.Status.active then
            ...
            if config.IS_CREATURE_SWARM_ENABLED then -- note: better to use a wave shader for ripples
                ...
                if math.abs(curr_state.creatures_vel_x[i] - prev_state.creatures_vel_x[i]) >= tolerance then
                    ...
                    local distorting_radius = lerp(creature_radius - 1,
                        creature_radius + (is_draw_segment and 1 + segment_distortion or 2), alpha)
            ...
            local is_away_from_corner = curr_x >= 0 + corner_offset and curr_x <= arena_w - corner_offset and curr_y >=
                0 + corner_offset and curr_y <= arena_h - corner_offset
            if is_away_from_corner or is_not_moving then
                local health = curr_state.creatures_health[i]
                local is_healing = curr_state.creatures_is_active[i] == common.Status.not_active and health >
                    common.HealthTransitions.healing and health <= common.HealthTransitions.healthy
                if is_healing then
                    ...
                    if smooth_alpha < config.PHI_INV then
                        ...
                        local radius_factor = (1 + smooth_alpha * juice_frequency *
                            lerp(1, juice_frequency_damper, smooth_alpha))
...
function draw_hud()
    ...
    LG.print(table.concat({ 'Level ' .. game_level, 'Total hit count ' .. laser_intersect_creature_counter,
        'Healed count ' .. laser_intersect_final_creature_counter,
        string.format('%.4s', game_timer_t) }, '\n'), 1 * pos_x + pad_x, 1 * pos_y + pad_y)

    if config.debug.is_development and config.debug.is_trace_hud then
        LG.print(table.concat({ 'invulnerability_timer ' .. cs.player_invulnerability_timer,
            ...
            1 * pos_x - (hud_w * 0.25) + pad_x, (arena_h * 0.5) - 1 * pos_y + hud_h + pad_y)
    end
...
function draw_debug_hud()
    ...
    LG.print(table.concat({ 'creatures.active: ' .. active_counter, 'creatures.count: ' .. #cs.creatures_x,
        'player.angle: ' .. cs.player_rot_angle, 'player.speed_x: ' .. cs.player_vel_x,
        'player.speed_y: ' .. cs.player_vel_y, 'player.x: ' .. cs.player_x,
        'player.y: ' .. cs.player_y, 'stats.canvases: ' .. stats.canvases,
        'stats.canvasswitches: ' .. stats.canvasswitches, 'stats.drawcalls: ' .. stats.drawcalls,
        'stats.drawcallsbatch: ' .. stats.drawcallsbatched, 'stats.fonts: ' .. stats.fonts,
        'stats.images: ' .. stats.images, 'stats.shaderswitches: ' .. stats.shaderswitches,
        'stats.texturememory: ' .. stats.texturememory, 'timer.dt: ' .. dt, 'timer.fps: ' .. fps,
...
function update_and_draw_player_shield_collectible(alpha)
    ...
    do -- draw collectible shield
        if shield_pos_x ~= nil and shield_pos_y ~= nil then
            ...
            local freq = 127 -- Hz
            local tween_fx = lerp(0.05, PHI_INV * math.sin(alpha * freq),
                alpha / math.min(0.4, game_timer_dt * math.sin(alpha)))
            ...
            LG.circle('fill', shield_pos_x, shield_pos_y,
...
local MAX_PARALLAX_ENTITIES = not config.IS_GAME_SLOW and (2 ^ 8) or (2 ^ 4)
...
local parallax_texture_path = ("resources/texture/tinycreatures_20241015134217.png") --- HACK: Add curved (bowl) depth, by using crt'd screenshot of previous gameplay.... decent results --with least effort
-- Load texture and create quads.
local parallax_texture = LG.newImage(parallax_texture_path or 'resources/texture/parallax_texture.png')
local parallax_quads = {}
local parallax_quad_size = 16 -- Each quad is 16x16 pixels
local parallax_texture_width = parallax_texture:getWidth()
local parallax_texture_height = parallax_texture:getHeight()
for y = 0, parallax_texture_height - parallax_quad_size, parallax_quad_size do
    for x = 0, parallax_texture_width - parallax_quad_size, parallax_quad_size do
        local quad = (LG.newQuad(x, y, parallax_quad_size, parallax_quad_size, parallax_texture_width, parallax_texture_height))
        table.insert(parallax_quads, quad)
    end
end
...
for i = 1, MAX_PARALLAX_ENTITIES do
    ...
    parallax_entity_quads[i] = love.math.random(1, #parallax_quads) -- Randomly assign a quad to each entity
end
...
-- DOING: Create parallax texture spritebatch that uses quads.
local parallax_texture_sprite_batch = LG.newSpriteBatch(parallax_texture, MAX_PARALLAX_ENTITIES)

local parallax_amplitude_x_factor   = 0.618 ^ 17
local parallax_entity_alpha_color   = ({ 0.56, 0.7, 1.0 })[config.CURRENT_THEME]
local parallax_offset_x             = 0
local parallax_offset_y             = 0
local parallax_smoothstep_sign1     = ({ -1, 1 })[love.math.random(1, 2)]
local parallax_smoothstep_sign2     = ({ -3, 3 })[love.math.random(1, 2)]

function update_background_shader(dt)
    local alpha = dt_accum * config.FIXED_DT_INV
    local a, b, t = parallax_smoothstep_sign1 * 0.003 * alpha, parallax_smoothstep_sign2 * 0.03 * alpha,
    ...
    local vel_x = 1 --- FIXME: TEMPORARY GLOBAL (debugging for smooth curves)
    do              -- Oscillate the x position using a sine function
        local oscillate_amplitude_x = parallax_amplitude_x_factor * love.math.random(1, PHI)
        local oscillate_frequency_x = love.math.random(PHI, 0)
        vel_x = vel_x * math.sin(game_timer_t * oscillate_frequency_x) * oscillate_amplitude_x * dt
    end
    local vel_y = math.abs(PHI_INV * 1.25 * freq) * dt

    for i = 1, MAX_PARALLAX_ENTITIES, 4 do
        if (config.IS_GRUG_BRAIN and config.IS_GAME_SLOW) and (screenshake.duration > 0) then
            vel_x = (vel_x - common.lerper['smoothstep'](vel_x * (-love.math.random(-4, 4)), vel_x * love.math.random(-4, 4), smoothValue))
            vel_y = (vel_y - common.lerper['smoothstep'](vel_y * (-love.math.random(-0.5, 2.5)), vel_y * love.math.random(0, 8), smoothValue))
        end
...
function draw_background_shader(alpha)
    ...
    if IS_PARALLAX_TARGET_PLAYER then
        parallax_offset_x = cs.player_x / arena_w -- TODO: should lerp on wrap
        parallax_offset_y = cs.player_y / arena_h
        dx = parallax_offset_x * PARALLAX_OFFSET_FACTOR_X
        dy = parallax_offset_y * PARALLAX_OFFSET_FACTOR_Y
    ...
    local is_quads = true
    if is_quads then
        local is_draw_quads_batch_of_4 = true
        if is_draw_quads_batch_of_4 then
            local is_sprite_batch = true
            if is_sprite_batch then
                parallax_texture_sprite_batch:clear()
                for i = 1, MAX_PARALLAX_ENTITIES, 4 do -- `4` TEMPORARY -> 145:(drawcallsbatch) and 27:(drawcalls)
                    parallax_texture_sprite_batch:add(parallax_quads[parallax_entity_quads[i]],
                    ...
                    (parallax_entity_pos_y[i] - (dy / parallax_entity_depth[i])) * arena_h, 0, 1, 1,
                        parallax_quad_size * .5, parallax_quad_size * .5)
                    parallax_texture_sprite_batch:add(parallax_quads[parallax_entity_quads[i + 1]],
                    ...
                    (parallax_entity_pos_y[i + 1] - (dy / parallax_entity_depth[i + 1])) * arena_h, 0, 1, 1,
                        parallax_quad_size * .5, parallax_quad_size * .5)
                    parallax_texture_sprite_batch:add(parallax_quads[parallax_entity_quads[i + 2]],
                    ...
                    (parallax_entity_pos_y[i + 2] - (dy / parallax_entity_depth[i + 2])) * arena_h, 0, 1, 1,
                        parallax_quad_size * .5, parallax_quad_size * .5)
                    parallax_texture_sprite_batch:add(parallax_quads[parallax_entity_quads[i + 3]],
                ...
                parallax_texture_sprite_batch:flush()
                LG.draw(parallax_texture_sprite_batch)
            else
                for i = 1, MAX_PARALLAX_ENTITIES, 4 do -- `4` TEMPORARY -> 280:(drawcallsbatch) and 27:(drawcalls)
                    LG.draw(parallax_texture, parallax_quads[parallax_entity_quads[i]],
                        (parallax_entity_pos_x[i] - (dx / parallax_entity_depth[i])) * arena_w,
                        (parallax_entity_pos_y[i] - (dy / parallax_entity_depth[i])) * arena_h, 0, 1, 1,
                        parallax_quad_size * .5, parallax_quad_size * .5)
                    LG.draw(parallax_texture, parallax_quads[parallax_entity_quads[i + 1]],
                        (parallax_entity_pos_x[i + 1] - (dx / parallax_entity_depth[i + 1])) * arena_w,
                        (parallax_entity_pos_y[i + 1] - (dy / parallax_entity_depth[i + 1])) * arena_h, 0, 1, 1,
                        parallax_quad_size * .5, parallax_quad_size * .5)
                    LG.draw(parallax_texture, parallax_quads[parallax_entity_quads[i + 2]],
                        (parallax_entity_pos_x[i + 2] - (dx / parallax_entity_depth[i + 2])) * arena_w,
                        (parallax_entity_pos_y[i + 2] - (dy / parallax_entity_depth[i + 2])) * arena_h, 0, 1, 1,
                        parallax_quad_size * .5, parallax_quad_size * .5)
                    LG.draw(parallax_texture, parallax_quads[parallax_entity_quads[i + 3]],
                        (parallax_entity_pos_x[i + 3] - (dx / parallax_entity_depth[i + 3])) * arena_w,
                        (parallax_entity_pos_y[i + 3] - (dy / parallax_entity_depth[i + 3])) * arena_h, 0, 1, 1,
                        parallax_quad_size * .5, parallax_quad_size * .5)
                end
            ...
            if is_sprite_batch then
                parallax_texture_sprite_batch:clear()
                for i = 1, MAX_PARALLAX_ENTITIES, 1 do -- `1` TEMPORARY -> 145:(drawcallsbatch) and 27:(drawcalls)
                    local quad = parallax_quads[parallax_entity_quads[i]]
                    ...
                    parallax_texture_sprite_batch:add(quad, pos_x, pos_y, 0, 1, 1, parallax_quad_size * .5,
                        parallax_quad_size * .5)
                end
                parallax_texture_sprite_batch:flush()
                LG.draw(parallax_texture_sprite_batch)
        else
            for i = 1, MAX_PARALLAX_ENTITIES, 1 do -- `1` TEMPORARY -> 280:(drawcallsbatch) and 27:(drawcalls)
                ...
                local pos_x = (parallax_entity_pos_x[i] - (dx / parallax_entity_depth[i])) * arena_w
                local pos_y = (parallax_entity_pos_y[i] - (dy / parallax_entity_depth[i])) * arena_h
                    LG.draw(parallax_texture, quad, pos_x, pos_y, 0, 1, 1, parallax_quad_size * .5,
                        parallax_quad_size * .5) -- draw with center pivot
                end
            end
        end
    else
...
function draw_screenshake_fx(alpha)
    if screenshake.duration > 0 then                                           -- vfx
        if screenshake.duration >= 0.125 and screenshake.duration <= 0.96 then -- snappy screenflash
            ...
            LG.setColor(
                ({ { 0.15, 0.15, 0.15, flash_alpha }, { 0.5, 0.5, 0.5, flash_alpha }, { 1, 1, 1, flash_alpha } })
...
function love.load()
    ...
    do                                                                                                              -- Music time
        ...
        if config.debug.is_development then
            music_bgm:setVolume(music_bgm:getVolume() * 0.125)
        end
        -- Master volume
    ...
    --[[ ORIGINAL PIPELINE
        post_processing = moonshine(arena_w, arena_h, fx.colorgradesimple) .chain(fx.chromasep) .chain(fx.crt) .chain(fx.scanlines) .chain(fx.vignette) .chain(fx.godsray),
    ]]
    ...
    if graphics_config.chromatic_abberation.enable then
        local mode_settings = { default = { angle = 0, radius = 0.0 }, minimal = { angle = 0, radius = 0.5 }, advanced = { angle = 180, radius = 1.2 } }

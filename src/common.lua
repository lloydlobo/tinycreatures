...
M.Color = {
    ...
    background = ({ { 0.05, 0.05, 0.05 }, { 0.4, 0.4, 0.4 }, { 0.75, 0.75, 0.75 } })[config.CURRENT_THEME], -- dark waters low_light ─ exposure: 0.0625, decay: 0.60
    creature_healed = { 0.75, 0.75, 0.75 },
    creature_healed_parallax = { 0.80, 0.80, 0.80 },
...
function M.round(x, bracket)
    ...
    return math.floor(x / bracket + M.sign(x) * .5) * bracket
end
...
do
    assert(M.approx(1.234, 1.230, .004))
    assert(not M.approx(1.234, 1.230, .005))
end
...
do
    ...
    assert(tostring(actual) == tostring(expected),
        'actual: ' .. tostring(actual) .. ': expected: ' .. tostring(expected)) -- FIXME: Fails without string cmp
end
...
M.lerper = {
    ...
    square = function(a, b, t)
        return (1 - (t * t)) * a + (t * t) * b
    end
}
do
    assert(M.lerper["linear"](0, 10, .5) == 5)
    assert(M.lerper["square"](0, 10, .5) == 2.5)
    assert(M.approx(M.lerper["sqrt"](0, 10, .25), 5))
    assert(M.approx(M.lerper["sqrt"](0, 10, .5), 7.37))
    assert(M.approx(M.lerper["sqrt"](0, 10, .75), 8.66))
    assert(M.lerper["cube"](0, 10, .5) == 1.25)
    assert(M.approx(M.lerper["cube"](0, 10, .75), 4.21))
    assert(M.round(M.lerper["log"](1, 10, .5), 0.2) == 3.2)
    assert(M.approx(M.lerper["log"](1, 10, .75), 5.62))
    assert(M.lerper["smoothstep"](1, 10, .5) == 5.5)
    assert(M.lerper["smoothstep"](1, 10, .25) == 2.40625)
    assert(M.lerper["smoothstep"](1, 10, .75) == 8.59375)
end

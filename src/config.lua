...
M = {
    ...
    MAX_PLAYER_HEALTH = 3,
    MAX_PLAYER_TRAIL_COUNT = -2 + math.floor(math.pi * math.sqrt(_player_radius * _phi_inv)), -- player_radius(32)*PHI==20(approx)
    PLAYER_ACCELERATION = 5 * ({ 150, 200, 300 })[speed_mode],
    PLAYER_CIRCLE_IRIS_TO_EYE_RATIO = _phi_inv,
    PLAYER_DEFAULT_TURN_SPEED = ({ (10 * _phi_inv), 10, 1 + (30 / 2) / 4 + (_player_acceleration / _fixed_fps) })[speed_mode],
